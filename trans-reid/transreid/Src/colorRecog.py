...
def validateYellow(cropped_frame: ndarray) -> bool:
    ...
    crop = lambda frm, cen, dim: frm[cen[0] - dim[0] // 2:cen[0] + dim[0] // 2,
                                 cen[1] - dim[1] // 2:cen[1] + dim[1] // 2]

    if dim[0] > dim[1]:
        dim[0] = dim[0] // 4  # Vertical Traffic Light
    else:
        dim[1] = dim[1] // 4  # Horizontal Traffic Light

    yellow_light = crop(cropped_frame, cen, dim)

    if maskColor(yellow_light, validateYellow=True) > 50:
        return True
    else:
        return False
...
def recognize_color(frame: ndarray, prediction: Tensor, print_info: bool = False):
    ...
    for n, traffic_light in enumerate(prediction):
        ...
        if (red > green) & (red > yellow) & (red > 50):
            if not validateYellow(traffic_light):
                color["red"].append((n, confidence))
            else:
                color["yellow"].append((n, confidence))
        elif (green > yellow) & (green > 50):
            color["green"].append((n, confidence))
        elif yellow > 50:
            if validateYellow(traffic_light):
                color["yellow"].append((n, confidence))
            else:
                color["red"].append((n, confidence))
        else:
...
def chooseOne(light_colors: Dict[str, List[Tuple[int, float]]]) -> Tuple[int, float] or None:
    if not light_colors['red']:
        return None
    chosen = [0, 0.0]
    for value in light_colors['red']:
        if value[-1] > chosen[-1]:
            chosen = list(value)
    return tuple(chosen)