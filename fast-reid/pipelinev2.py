# pipeline with red light violation
...
class VehicleTracker:
        ...
        def mouse_callback(event, x, y, flags, param):
            ...
            if event == cv2.EVENT_LBUTTONDOWN:
                ...
                if drawing and start_point:
                    line_points.append((start_point[0], start_point[1], current_point[0], current_point[1]))
    ...
    def update_tracks(self, detections, features, frame_num, chosen, frame):
        ...
        for detection_idx, track_id in enumerate(detections.tracker_id):
            ...
            # Violation detection
            if chosen is not None and chosen[1] > 0:  # Red light detected with confidence
                cx = (detections.xyxy[detection_idx][0] + detections.xyxy[detection_idx][2]) / 2
                cy = (detections.xyxy[detection_idx][1] + detections.xyxy[detection_idx][3]) / 2
        ...
        if frame_violator_ids:
            ...
            snapshot_path = os.path.join(self.snapshot_dir, f"{timestamp}_{violator_ids_str}.jpg")
    ...
    def process_frame(self, frame, frame_num):
        ...
        if len(traffic_light_detections) > 0:
            traffic_light_boxes = torch.tensor(traffic_light_detections.xyxy, dtype=torch.float32)
            conf = torch.ones((len(traffic_light_detections), 1))
            cls = torch.full((len(traffic_light_detections), 1), self.TRAFFIC_LIGHT_CLASS, dtype=torch.float32)
        ...
        for i, track_id in enumerate(vehicle_detections.tracker_id):
            if track_id in self.tracks:
                ...
                labels.append("ID: ?")
        annotated_frame = self.box_annotator.annotate(annotated_frame, detections=vehicle_detections)
